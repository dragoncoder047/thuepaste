function chanceOfHalting(thue, timeout = 5000) {
    const start = +new Date();
    function hc(text, seenRules) {
        if (+new Date() - start > timeout) throw 'timeout';
        var matches = thue.matches(text);
        if (matches.length === 0) return 1.0; // will defintely halt.
        var chances = [];
        for (var [r, m] of matches) {
            var applied = thue.apply(r, m, text, true);
            // test to see if the rule will match itself or in a cycle
            var oldRules = thue.rules;
            var moreSeen = seenRules.indexOf(r) === -1 ? [r].concat(seenRules) : seenRules;
            thue.rules = moreSeen;
            var cmatch = thue.matches(applied).length;
            thue.rules = oldRules;
            if (cmatch > 0)
                chances.push(1.0 - (cmatch / matches.length)); // may match itself this many times, so it might not halt.
            else
                chances.push(hc(thue.apply(r, m, text, true), moreSeen));
        }
        return chances.reduce((a, b) => a + b, 0.0) / chances.length;
    }
    return hc(thue.text, []);
}
